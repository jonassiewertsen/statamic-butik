...
class ExpressCheckoutDeliveryTest extends TestCase
{
    ...
    /** @test */
    public function the_variant_information_will_be_show()
    {
        $variant = create(Variant::class, ['product_slug' => $this->product->slug])->first();

        $this->get(route('butik.checkout.express.delivery', ['product' => $this->product, 'variant' => $variant->original_title]))
            ->assertSee($variant->title)
            ->assertSee($variant->price)
            ->assertSee($variant->total_price)
            ->assertSee($variant->tax_amount)
            ->assertSee($variant->tax_percentage);
    }

    /** @test */
    public function a_not_exisiting_variant_will_show_a_404_page()
    {
        $this->get(route('butik.checkout.express.delivery', ['product' => $this->product, 'variant' => 'not-existing']))
            ->assertNotFound();
    }
